...
const pesquisarJogo = () => {
  ...
  jogosFiltrados.value = listaJogos.value.filter(j => j && j.NomeJogo?.toLowerCase().includes(jogoPesquisa.value.toLowerCase()))
}
...
const pesquisarCliente = async () => {
  ...
  clientesFiltrados.value = listaClientes.value.filter(c => c.nome.toLowerCase().includes(clientePesquisa.value.toLowerCase()))
}
...
const carregarReserva = async () => {
  ...
  reserva.unidade_id = d.unidade_id || ''
  reserva.forma_pagamento = d.forma_pagamento || ''
...
onMounted(async () => {
  loading.value = true
  await Promise.all([carregarJogos(), carregarClientes(), carregarSalas(), carregarUnidades()])
